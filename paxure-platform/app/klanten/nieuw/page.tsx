...
export default function NieuweKlantPage() {
  ...
  useEffect(() => {
    async function loadVerantwoordelijken() {
      setLoading(true)
      try {
        ...
        setError('Fout bij laden van verantwoordelijken')
      } finally {
        setLoading(false)
      }
    }

    loadVerantwoordelijken()
  }, [supabase])
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : 'Klant opslaan mislukt')
      setSubmitting(false)
    }
  }
    ...
    <div className="container mx-auto py-8 max-w-4xl">
      <div className="mb-8">
        ...
        <p className="text-muted-foreground">
          Maak een nieuwe klant aan met bijbehorende flow informatie
        </p>
      </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="flex justify-end gap-4">
              ...
              <Button type="submit" disabled={submitting}>
                {submitting ? 'Opslaan...' : 'Klant Opslaan'}
              </Button>
            </div>
          </form>
        </CardContent>
