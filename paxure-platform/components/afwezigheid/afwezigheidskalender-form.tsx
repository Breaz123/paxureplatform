...
interface AfwezigheidskalenderFormProps {
...
function formatWerkdagen(werkdagen: number | null | undefined): string {
  if (!werkdagen || werkdagen === 5) return '5/5'
  return `${werkdagen}/5`
}

function formatWerkdagenInfo(medewerker: Profile | undefined): string {
  if (!medewerker) return ''
  
  const parts: string[] = []
  
  if (medewerker.werkdagen_per_week && medewerker.werkdagen_per_week !== 5) {
    parts.push(formatWerkdagen(medewerker.werkdagen_per_week))
  }
...
export default function AfwezigheidskalenderForm({
  initialVerlof,
  medewerkers,
  canEdit,
  currentUserId,
}: AfwezigheidskalenderFormProps) {
  ...
  return (
    <div className="space-y-6">
        ...
        <Card>
          <CardHeader>
            <div className="flex items-center justify-between">
              <div>
                <CardTitle>Afwezigheid Toevoegen</CardTitle>
                <CardDescription>
                    ...
                    >
                      ...
                      <SelectContent>
                        {medewerkers.map((medewerker) => {
                          const werkdagenInfo = formatWerkdagenInfo(medewerker)
                          return (
                            <SelectItem key={medewerker.id} value={medewerker.id}>
                              <div className="flex items-center justify-between w-full">
                                <span>{medewerker.full_name || medewerker.email}</span>
                                {werkdagenInfo && (
      ...
      <Card>
        <CardHeader>
          <CardTitle>Afwezigheid Overzicht</CardTitle>
